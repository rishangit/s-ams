...
export class Company {
  ...
  static async create(companyData) {
    ...
    try {
      ...
      return { id: result.lastID, ...companyData, status: 'pending' }
    } catch (error) {
  ...
  static async update(id, companyData) {
    ...
    const query = `
      ...
      SET name = ?, address = ?, phone_number = ?, land_phone = ?, 
          geo_location = ?, updated_at = datetime('now')
      WHERE id = ?
    `
  ...
  static async findByUserAppointments(userId) {
    console.log('Company.findByUserAppointments (SQLite) called with userId:', userId)
    ...
    try {
      console.log('Executing SQLite query with userId:', userId)
      const rows = await db.all(query, [userId])
      console.log('SQLite query result:', rows.length, 'companies found')
      return rows
    } catch (error) {
